...
Turn concrete nouns and places in the user's request into visible interface structure and scenery, not merely themeColors. A forest request must visibly include elements such as trees/trunks, canopy or foliage, branches, paths, clearings, forest-floor texture, or layered depth; choose a coherent subset. Express the requested world in at least three distinct ways across layout structure, CSS/DOM decoration, SVG assets, typography/materials, and interaction. It must be recognizable in the first viewport even with the artwork hidden. Never return a generic card grid whose only thematic treatment is a matching background color; make artwork containers and navigation belong to the metaphor.
